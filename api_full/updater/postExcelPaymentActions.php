...
$uniqueStepsCount = isset($_POST['uniqueStepsCount']) ? (int)$_POST['uniqueStepsCount'] : 0;

$pdo = getDbConnection();

// Получаем последнее сохранённое значение steps_count из excel_steps_count
$stmtLast = $pdo->query("SELECT steps_count FROM excel_steps_count ORDER BY id DESC LIMIT 1");
$lastRow = $stmtLast->fetch(PDO::FETCH_ASSOC);
$dbStepsCount = $lastRow ? (int)$lastRow['steps_count'] : null;

// Получаем текущее количество steps
$stmt = $pdo->query("SELECT * FROM steps WHERE in_excel = true");
$rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
$currentUniqueStepIds = array_column($rows, 'id');
$currentUniqueStepsCount = count(array_unique($currentUniqueStepIds));

// Проверяем совпадение всех трёх значений
if ($uniqueStepsCount !== $currentUniqueStepsCount || $dbStepsCount !== $currentUniqueStepsCount) {
    echo json_encode([
        'status' => 'error',
        'message' => 'Количество steps не совпадает'
    ]);
    exit;
}

// Обновить in_excel = false, status = 3
