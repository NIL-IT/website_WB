...
try {
    ...
    $stmt = $pdo->prepare("
        SELECT s.id AS step_id, s.id_product, s.id_usertg AS step_user_id, s.step, s.image1, s.image2, s.image3, s.image4, s.image5, s.image6, s.updated_at,
            u1.username AS step_username, 
            p.tg_nick, p.name AS product_name, p.image_path AS product_image_path, p.article, p.expire,
    ...
    $values = [["# Отчёта", "ID покупателя", "Username покупателя", "ID продавца", "Username продавца", "Название товара", "Артикул товара", "Товар в выдаче?", "Путь до изображения товара", "Номер шага", "Дата перехода на 1 шаг", "Ссылка на отчёт", "Cкрин корзины", "Скрин товара продавца", "Скрин заказа в `Доставке`", "Скриншот о доставке", "Скриншот отзыва", "Фотография с штрих-кодом на фоне товара"]];
